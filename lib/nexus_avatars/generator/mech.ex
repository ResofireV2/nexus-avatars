defmodule NexusAvatars.Generator.Mech do
  ...
  def render(username) do
    ...
    {bg, acc, bright, dark, mid, _acc2} =
      Enum.at(@palettes, rng(seed, 0, length(@palettes)))
  ...
  # ── splitmix32 PRNG ────────────────────────────────────────────────────────
  # Returns a deterministic integer in 0..(range-1) for a given seed+salt.
  defp rng(seed, salt, range) do
    h  = Bitwise.bxor(seed, salt * 0x9e3779b9) |> Bitwise.band(0xFFFFFFFF)
    ...
    z3 = Bitwise.bxor(z2, Bitwise.bsr(z2, 16)) |> Bitwise.band(0xFFFFFFFF)
    ...
    w3 = Bitwise.bxor(w2, Bitwise.bsr(w2, 16)) |> Bitwise.band(0xFFFFFFFF)
    rem(w3, range)
  end

  # Jitter: continuous offset in 0..(range-1) using salt+100 to avoid
  # collisions with layout-selection calls that use salts 0..10.
  ...
  # Single eye renderer — 8 iris styles
  ...
  defp eye_hex_iris(cx, cy, sz, acc, bright, dark, mid) do
    ...
    ~s(<polygon points="#{cx},#{cy-sz+4} #{cx+sz-6},#{cy-3} #{cx+sz-6},#{cy+3} #{cx},#{cy+sz-4} #{cx-sz+6},#{cy+3} #{cx-sz+6},#{cy-3}" fill="#{acc}"/>) <>
  ...
  defp five_x_eyes(seed, acc, bright, dark, mid, eye_y, spread, sz) do
  ...
  defp brow_riveted(by, dark, mid) do
    ...
    ~s(<rect x="36" y="#{by-2}" width="184" height="18" rx="6" fill="#{dark}"/>) <> rivets
  end

  defp brow_sensor_dots(seed, by, acc, dark) do
    dots = Enum.map_join(0..10, "\n", fn i ->
      opacity = Float.round(0.35 + i * 0.055, 2)
      ~s(<circle cx="#{52+i*15}" cy="#{by+7}" r="3.5" fill="#{acc}" opacity="#{opacity}"/>)
    end)
    ~s(<rect x="42" y="#{by}" width="172" height="15" rx="6" fill="#{dark}"/>) <> dots
  end
  ...
  defp forehead_sensor_bar(seed, acc, dark, mid) do
    dots = Enum.map_join(0..6, "\n", fn i ->
  ...
  defp forehead_tri_antenna(acc, bright, dark) do
    [-30, 0, 30]
    |> Enum.with_index()
    |> Enum.map_join("\n", fn {dx, i} ->
      base_y = 24 - i * 6
  ...
  defp mouth_grill(seed, my, mw, mx, acc, dark) do
    ...
    gap  = div(mh, bars)
    bar_svgs = Enum.map_join(0..(bars-1), "\n", fn i ->
  ...
  defp chin_data_bar(seed, cy, acc, dark) do
    Enum.map_join(0..11, "\n", fn i ->
  ...
  defp chin_segmented_cells(seed, cy, acc, dark, mid) do
    Enum.map_join(0..3, "\n", fn i ->
      fill = if rem(i, 2) == 0, do: acc, else: mid
  ...
  defp extras(seed, acc, bright, dark, mid, eye_y) do
  ...
  defp extras_corner_fans(acc, dark, mid) do
    ...
    |> Enum.map_join("\n", fn {cx, cy} -> fan_svg(cx, cy, acc, dark, mid) end)
  end
  ...
  defp extras_side_pipe(seed, dark, mid) do
    ...
    rings = Enum.map_join([82, 112, 144, 174], "\n", fn y ->
